...
def imdb(text_field, label_field, args, **kargs):
    datafields = [("text",text_field),("label",label_field)]
    trn, val, tst = data.TabularDataset.splits(path='data', train='train_IMDB.csv', validation='val_IMDB.csv', 
                                               test='test_IMDB.csv', format='csv', fields=datafields)
...
def ag(text_field, label_field, args, **kargs):
    datafields = [("text",text_field), ("label",label_field)]
    trn, val, tst = data.TabularDataset.splits(path='data', train='train_AG.csv', validation='val_AG.csv', 
                                               test='test_AG.csv', format='csv', fields=datafields)
